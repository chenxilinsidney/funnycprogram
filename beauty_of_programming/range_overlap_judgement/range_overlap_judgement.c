...
TYPE range_overlap_judgement(TYPE* source, TYPE* target, TYPE count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        x1 = target[i << 1];
        y1 = target[(i << 1) + 1];
        DEBUG_PRINT_VALUE("%d", x1);
        DEBUG_PRINT_VALUE("%d", y1);
        ...
        for (j = i + 1; j < count; j++) {
            ...
            x2 = target[j << 1];
            y2 = target[(j << 1) + 1];
            DEBUG_PRINT_VALUE("%d", x2);
            DEBUG_PRINT_VALUE("%d", y2);
            /// process two relative range
            if (x2 >= x1 && x2 <= y1) {
                if (y1 <= y2) target[(i << 1) + 1] = y2;
                flag_targe_done[j] = 1;
            } else if (y2 >= x1 && y2 <= y1) {
                if (x2 <= x1) target[i << 1] = x2;
                flag_targe_done[j] = 1;
            } else if(x2 <= x1 && y1 <= y2) {
                target[i << 1] = x2;
                target[(i << 1) + 1] = y2;
            ...
            /// refresh x1 and y1 value
            x1 = target[i << 1];
            y1 = target[(i << 1) + 1];
        }
    }
    // display the processed range and flag
    ...
    for (i = 0; i < count; i++) {
        ...
        if (flag_targe_done[i])
            continue;
        /// test if source range in the target range
        if (target[i << 1] <= source[0] &&
