        ...
        int preorder_end_index)
{
    ...
    int inorder_index = inorder_start_index;
    while (inorder_index <= inorder_end_index &&
            inorder[inorder_index] != preoder[preorder_start_index]) {
        inorder_index++;
    }
    temp->key = preoder[preorder_start_index];
#ifndef ONLINE_JUDGE
    cout << "key:" << temp->key << endl;
    cout << "index:" << inorder_index << endl;
#endif
    ...
    temp->lchild = build_treeinpre(inorder,
            ...
            inorder_index - inorder_start_index + preorder_start_index);
    // right child
...
void traversal_tree_path_readonly(const int* inorder,
        int inorder_start_index,
        int inorder_end_index,
        int num_node,
        const int* postorder,
        ...
        int* leaf_key)
{
    ...
    if (num_node == 1) {
        sum_path += inorder[inorder_start_index];
        if (*least_path > sum_path) {
            *least_path = sum_path;
            *leaf_key = inorder[inorder_start_index];
        } else if (*least_path == sum_path &&
                *leaf_key > inorder[inorder_start_index]) {
            *leaf_key = inorder[inorder_start_index];
        }
        return;
    ...
    sum_path += postorder[postorder_end_index];
    // left 
    traversal_tree_path_readonly(inorder,
            inorder_start_index, inorder_index - 1,
            inorder_index - inorder_start_index,
            postorder,
            postorder_start_index,
...
int main(void)
{
    ...
    while (getline(cin, line_inorder) && getline(cin, line_postorder)) {
        ...
        int leaf_key = 0;
        traversal_tree_path_readonly(inorder, 0, number - 1, number,
                postorder, 0, number - 1, 0, &least_path, &leaf_key);
        cout << leaf_key << endl;
    }
}
